# TODO: What is this, why not merge with cam_utils
import cv2
import numpy as np
import os
...
def read_camera(intri_name, extri_name, cam_names=[]):
    ...
    for cam in cam_names:
        ...
        rdist = intri.read('rdist_{}'.format(cam))
        if rdist is not None:
            print('[camera] use radial dist for {}'.format(cam))
            cams[cam]['rdist'] = rdist
...
def get_fundamental_matrix(cameras, basenames):
    def skew_op(x): return np.array([[0, -x[2], x[1]], [x[2], 0, -x[0]], [-x[1], x[0], 0]])

    def fundamental_op(K_0, R_0, T_0, K_1, R_1, T_1): return np.linalg.inv(K_0).T @ (
        R_0 @ R_1.T) @ K_1.T @ skew_op(K_1 @ R_1 @ R_0.T @ (T_0 - R_0 @ R_1.T @ T_1))
    def fundamental_RT_op(K_0, RT_0, K_1, RT_1): return fundamental_op(K_0, RT_0[:, :3], RT_0[:, 3], K_1,
                                                                       RT_1[:, :3], RT_1[:, 3])
