...
from easyvolcap.utils.gaussian_utils import convert_to_gaussian_camera, convert_to_gl_camera, render_fdgs, render_diff_gauss, in_frustum
...
def test_diff_gauss_frustum_culling():
    ...
    ind = in_frustum(xyz, camera.full_proj_transform).nonzero()[..., -1]
    export_pts(xyz[ind], rgb[ind], filename='in_frustum.ply')


def test_fast_gauss_frustum_culling():
    camera = convert_to_gl_camera(K, R, T, H, W, n, f,
                                  K.cpu(), R.cpu(), T.cpu(), H.cpu(), W.cpu(), n.cpu(), f.cpu()
                                  )

    export_camera(affine_inverse(affine_padding(torch.cat([R, T], dim=-1))), K, filename='camera.ply')
    ray_o, ray_d = get_rays(H, W, K, R, T, z_depth=True, correct_pix=True)
    export_pts(ray_o + ray_d * n, filename='near_plane_fast_gauss.ply')
    export_pts(ray_o + ray_d * f, filename='far_plane_fast_gauss.ply')
    export_pts(xyz, rgb, filename='xyz_fast_gauss.ply')
    ind = in_frustum(xyz, camera.full_proj_transform).nonzero()[..., -1]
    export_pts(xyz[ind], rgb[ind], filename='in_frustum_fast_gauss.ply')
