...
def main():
    args = dotdict(
        data_root='data/datasets/GameSynthetic/wukong/wukong_xietao_250116/250116_012703_254/check_dir/verify/vggsfm',
        colmap='sparse',
        output='cameras/00',
    ...
    for key in cameras.keys():
        ...
        if cameras[key].model == 'SIMPLE_PINHOLE':
            K = np.array([[p[0], 0, p[1], 0, p[0], p[2], 0, 0, 1]]).reshape(3, 3)
            dist = np.array([[0., 0., 0., 0., 0.]])
        elif cameras[key].model == 'PINHOLE':
            K = np.array([[p[0], 0, p[2], 0, p[1], p[3], 0, 0, 1]]).reshape(3, 3)
            dist = np.array([[0., 0., 0., 0., 0.]])
        elif cameras[key].model == 'SIMPLE_RADIAL':
            f, cx, cy, k = p
            K = np.array([f, 0, cx, 0, f, cy, 0, 0, 1]).reshape(3, 3)
            dist = np.array([[k, 0, 0, 0, 0]])
        elif cameras[key].model == 'RADIAL':
            f, cx, cy, k1, k2 = p
            K = np.array([f, 0, cx, 0, f, cy, 0, 0, 1]).reshape(3, 3)
    ...
    easycams = {}
    for key, val in sorted(images.items(), key=lambda item: item[1].name):
        if args.sub in val.name:
            ...
            cam['R'] = R
            cam['T'] = t * args.scale
            # NOTE: The name of the camera is the name of the image
