...
def main():
    ...
    parser.add_argument('--camera_model', choices=['OPENCV', 'RADIAL_FISHEYE'], default='OPENCV')
    ...
    for frame in frames:
        ...
        for cam_id, cam_name in enumerate(basenames):
            ...
            tar = join(output_dir, 'images', f'{cam_id:02d}{args.image_ext}')
            os.symlink(relpath(src, dirname(tar)), tar)
            src = join(args.data_root, args.mask_dir, cam_name, f'{frame}{args.mask_ext}')
            tar = join(output_dir, 'masks', f'{cam_id:02d}{args.mask_ext}')
            os.symlink(relpath(src, dirname(tar)), tar)
